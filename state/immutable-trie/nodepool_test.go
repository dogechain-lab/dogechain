...
func TestNodePool_RecycleObject(t *testing.T) {
	np := NewNodePool()

	ptrMap := make(map[uintptr]interface{})
	ptrExist := func(obj interface{}) bool {
		ptr := reflect.ValueOf(obj).Pointer()
		_, exist := ptrMap[ptr]
	...
	{
		node := np.GetFullNode()
		binary.BigEndian.PutUint64(node.hash[:8], uint64(0xffffffffffffffff))
		...
		ptrMap[reflect.ValueOf(node).Pointer()] = node
		np.PutNode(node)
	}

	{
		...
		binary.BigEndian.PutUint64(node.hash[:8], uint64(0xffffffffffffffff))
		binary.BigEndian.PutUint64(node.key[:8], uint64(0xffffffffffffffff))
...

	{
		...
		// fill object
		node.hash = true
		binary.BigEndian.PutUint64(node.buf[:8], uint64(0xffffffffffffffff))
